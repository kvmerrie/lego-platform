...
This document describes the current Supabase foundation for real auth and owned or wanted persistence. The repo now has browser and API wiring in place, with email and password as the primary sign-in path, Google as the first social option, and magic link kept as a secondary fallback.
...
- The web auth surface now offers email and password as the primary account flow backed by Supabase Auth.
- Google OAuth is wired as the first social sign-in path when the provider is enabled in Supabase for the current environment.
- Magic link remains available as a fallback when a collector prefers not to use a password.
- Email confirmation links, recovery links, OAuth redirects, and magic links all return through `/auth/callback`, where the browser session is completed explicitly before the app redirects back to the intended page.
...
6. In the Supabase dashboard, enable email and password auth for the project.
7. If Google sign-in should be available, enable the Google provider and configure the matching OAuth client credentials in Supabase.
8. Configure the auth site URL and redirect URLs so the local web app can receive the sign-in callback at `http://localhost:3000/auth/callback`.
...
- keep the Google provider disabled in environments where its OAuth client is not configured yet rather than surfacing a half-wired social sign-in path
...
- successful password sign-in, Google sign-in, email confirmation, password recovery, and magic link flows should all briefly land on `/auth/callback` before returning to the intended catalog or collector page.
...
The current UI and web routes remain unchanged. The `/api/v1` handlers now use Supabase-backed repositories, the web auth surface supports sign-in, sign-up, password recovery, Google sign-in, magic-link fallback, and sign-out, and signed-in collectors can edit a compact product-facing profile while broader account-management work still waits for a later phase.
