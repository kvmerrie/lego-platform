...
- keep `UNIEKE_BRICKS_FEED_URL`, `UNIEKE_BRICKS_FEED_ORIGIN_RESOLVE_IP`, `UNIEKE_BRICKS_MERCHANT_SLUG`, `UNIEKE_BRICKS_MERCHANT_NAME`, `SUPABASE_URL`, and `SUPABASE_SERVICE_ROLE_KEY` scoped to the scheduled job
- `UNIEKE_BRICKS_FEED_URL` is the public WooCommerce Product Feed Pro XML URL and must stay HTTPS on `uniekebricks.nl`
- set `UNIEKE_BRICKS_FEED_ORIGIN_RESOLVE_IP=93.119.2.137` in Render to mimic `curl --resolve uniekebricks.nl:443:93.119.2.137 https://uniekebricks.nl/...`
- resolve-IP requests keep URL host, TLS SNI, and `Host` on `uniekebricks.nl`, but force the TCP DNS lookup to the configured origin IP
- resolve-IP requests include browser-like XML request headers; the job logs `origin_mode=resolve_ip`, `request_url_host` and `resolve_ip` without printing the full feed URL
- `UNIEKE_BRICKS_FEED_ORIGIN_URL` and `UNIEKE_BRICKS_FEED_ORIGIN_HOST_HEADER` are legacy fallback knobs and should be removed or ignored in Render once `UNIEKE_BRICKS_FEED_ORIGIN_RESOLVE_IP` is set
- 404 from the origin route is treated as a recoverable upstream failure so the run ends degraded with `import_skipped=true` and `latest_rows_marked_stale=0`
- the importer only runs after the response looks like XML; HTML/Cloudflare challenge responses are treated as recoverable upstream failures and do not mark stale rows
...
- use `UNIEKE_BRICKS_FEED_ORIGIN_RESOLVE_IP='93.119.2.137' pnpm sync:uniekebricks-feed -- --dry-run --max-products 20 --debug-samples 3` to validate the production origin route locally
