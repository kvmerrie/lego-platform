...
import { syncMisterBricksFeed } from '@lego-platform/api/data-access-server';
import {
...
async function main() {
  ...
  console.log(
    `[misterbricks-feed-sync] start source=channable merchant=misterbricks mode=${dryRun ? 'dry-run' : 'write'} debug_samples=${debugSamples ?? 0} debug_unmatched_samples=${debugUnmatchedSamples ?? 0} max_products=${maxProducts ?? 0} report_unmatched_path=${JSON.stringify(reportUnmatchedPath ?? '')}`,
  );
