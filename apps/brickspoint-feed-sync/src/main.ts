...
async function main() {
  ...
  if (!hasServerSupabaseConfig() && !dryRun) {
    throw new Error(
      `Brickspoint feed sync requires Supabase server access. Missing: ${getMissingServerSupabaseEnvKeys().join(', ')}.`,
    );
  }

  if (!hasServerSupabaseConfig() && reportStaleLatestPath) {
    throw new Error(
      `Brickspoint stale latest report requires Supabase server access. Missing: ${getMissingServerSupabaseEnvKeys().join(', ')}.`,
    );
  }

  if (!hasBrickspointFeedConfig()) {
    throw new Error(
      `Brickspoint feed sync requires a product feed URL. Missing: ${getMissingBrickspointEnvKeys().join(', ')}.`,
    );
  }

  console.log(
    `[brickspoint-feed-sync] start source=direct merchant=brickspoint origin_mode=public mode=${dryRun ? 'dry-run' : 'write'} debug_samples=${debugSamples ?? 0} debug_unmatched_samples=${debugUnmatchedSamples ?? 0} max_products=${maxProducts ?? 0} report_unmatched_path=${JSON.stringify(reportUnmatchedPath ?? '')} report_stale_latest_path=${JSON.stringify(reportStaleLatestPath ?? '')}`,
  );
  ...
  if (result.debugInfo) {
    console.log(
      `[brickspoint-feed-sync] debug_samples fetched_products=${result.debugInfo.fetchedProductCount} lego_candidates=${result.debugInfo.legoCandidateCount} sample_count=${result.debugInfo.sampleCount} parse_failures=${result.debugInfo.parseFailureCount}`,
    );
    console.log(
      JSON.stringify(
        {
  ...
  if (result.unmatchedDebug) {
    console.log(
      `[brickspoint-feed-sync] debug_unmatched total_rows=${result.unmatchedDebug.totalUnmatchedRows} unique_sets=${result.unmatchedDebug.uniqueUnmatchedSetCount} sample_count=${result.unmatchedDebug.sampleRows.length}`,
    );
    console.log(
      JSON.stringify(
        {
    ...
    if (reportUnmatchedPath) {
      ...
      console.log(
        `[brickspoint-feed-sync] unmatched_report_written path=${JSON.stringify(reportUnmatchedPath)} rows=${result.unmatchedDebug.totalUnmatchedRows} unique_sets=${result.unmatchedDebug.uniqueUnmatchedSetCount}`,
      );
    }
  }
  ...
  if (reportStaleLatestPath) {
    ...
    console.log(
      `[brickspoint-feed-sync] stale_latest_report_written path=${JSON.stringify(reportStaleLatestPath)} rows=${result.existingStaleSuccessLatestCount} duplicate_seed_count=${result.existingStaleSuccessLatestDuplicateSeedCount ?? 0} missing_from_feed_count=${result.existingStaleSuccessLatestMissingFromFeedCount ?? 0}`,
    );
  }
  ...
  if (!dryRun && result.changedSetIds.length > 0) {
    try {
      const revalidationResult = await revalidatePublicCatalogPriceChanges({
        ...
        reason: 'brickspoint_feed_sync',
      });
      console.log(
        `[brickspoint-feed-sync] revalidation attempted=${revalidationResult.attempted} skipped=${revalidationResult.skipped} changed_set_count=${result.changedSetIds.length} revalidated_set_path_count=${result.changedSetSlugs.length} path_count=${revalidationResult.pathCount} tag_count=${revalidationResult.tagCount}`,
      );
    } catch (error) {
      console.warn('[brickspoint-feed-sync] revalidation warning', {
