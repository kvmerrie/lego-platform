...
import { syncTradeTrackerCoppenswarenhuisFeed } from '@lego-platform/api/data-access-server';
import {
...
async function main() {
  ...
  console.log(
    `[coppenswarenhuis-feed-sync] start source=tradetracker merchant=coppenswarenhuis mode=${dryRun ? 'dry-run' : 'write'} debug_samples=${debugSamples ?? 0} debug_unmatched_samples=${debugUnmatchedSamples ?? 0} max_products=${maxProducts ?? 0} report_unmatched_path=${JSON.stringify(reportUnmatchedPath ?? '')}`,
  );
...
main().catch((error) => {
  console.error(
    '[coppenswarenhuis-feed-sync] failed source=tradetracker merchant=coppenswarenhuis',
  );

  if (error instanceof Error) {
    console.error(`[coppenswarenhuis-feed-sync] error=${error.message}`);
  }
