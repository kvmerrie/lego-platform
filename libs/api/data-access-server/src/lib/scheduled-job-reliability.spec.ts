...
describe('scheduled job reliability', () => {
  ...
  test('treats resolve-IP 404 feed failures as recoverable degraded runs', () => {
    expect(
      classifyScheduledJobFailure(
        new Error(
          'Unieke Bricks feed request failed with 404 Not Found. upstream_http origin_mode=resolve_ip status_code=404',
        ),
      ),
    ).toEqual({
      exitCode: 0,
